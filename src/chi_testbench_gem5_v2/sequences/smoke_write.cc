/*
 * Copyright (c) 2026 ripopov
 * SPDX-License-Identifier: BSD-3-Clause
 */

#include <cstdint>

#include "chi_testbench_gem5_v2/driver.hh"
#include "chi_testbench_gem5_v2/sequence_context.hh"
#include "chi_testbench_gem5_v2/sequences/registry.hh"

namespace gem5
{
namespace chi_gem5tb_v2
{
namespace
{

// smoke_write — blocking CHI WriteUniqueFull to each configured
// address, `iterations` times. The tile is non-HN so it forwards the
// write to its downstream HN-F; the driver receives CompDBIDResp from
// the HN (via the tile) and responds with NCBWrData.
void
smoke_write_seq(SequenceContext &ctx)
{
    ChiDriverNode &d = ctx.drv;
    const auto &p = d.params();
    const uint32_t access_size = p.access_size ? p.access_size : 8;
    const uint32_t iterations = p.iterations ? p.iterations : 1;

    uint8_t buf[64] = {0};
    for (uint32_t i = 0; i < sizeof(buf); ++i) {
        buf[i] = (uint8_t)(0xA0 + (i & 0xF));
    }

    for (uint32_t it = 0; it < iterations; ++it) {
        for (auto addr : p.addresses) {
            d.write_unique_full(addr, buf,
                                access_size <= sizeof(buf) ? access_size : 8);
        }
    }
}

[[maybe_unused]] Registrar _r("smoke_write", &smoke_write_seq);

} // namespace
} // namespace chi_gem5tb_v2
} // namespace gem5
