...
parser.add_argument(
    ...
    default=500_000_000,
    help=(
        "length of each bounded Linux phase. A JitCPU phase advances "
        "simulated time far faster than the guest retires instructions, so "
        "the kernel returns to O3 with a backlog of RTC interrupts to "
        "service. The phase must outlast that catch-up, or the O3 phase "
        "retires only kernel instructions and the userspace-progress check "
        "fails."
    ),
)
